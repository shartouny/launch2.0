...
import trimWhiteSpace from './canvas/trimWhiteSpace';
import fitInBox from './canvas/fitInBox';
...
const preprocessCanvas = async (canvas, config) => {
  config.statusCallback(10, 'Checking for transparency');
  ...
  // fit in box
  if(config.scaleToMaxDimensions){
    canvas = fitInBox(canvas, config.scaleToMaxDimensions.width, config.scaleToMaxDimensions.height);
  }
  await nextFrame();
...
const processSign = (src, options = {}) => {
  return new Promise(async (resolve, reject) => {
    ...
    const config = {
      ...
      scaleToMaxDimensions: options.scaleToMaxDimensions || null
    };
    if (src) {
      ...
      image.onload = async () => {
        ...
        config.statusCallback(80, 'Tracing image. This could take a while.');
        await nextFrame();
        ...
        svgson.children.sort((b, a) => (a.attributes.d.length > b.attributes.d.length) ? 1 : ((b.attributes.d.length > a.attributes.d.length) ? -1 : 0));
        let numSections = 0;

        // remove the white (background) shapes.
        svgson.children = svgson.children.map(v => {
          const rgb = v.attributes.fill;
          const [r, g, b] = rgb.substring(4, rgb.length - 1)
            .replace(/ /g, '')
            .split(',')
            .map(x => parseInt(x));
          const mean = (r + g + b) / 3;
